...
{
    public partial class InvoiceSale : Form
    {
        MainMenu frmmain;
        ...
        DataTable dtjual; DataTable dtpakaian;
        string query; int count;

        public InvoiceSale()
        {
            InitializeComponent();
            lb_idjual.BackColor = ColorTranslator.FromHtml("#223559");
            lb_idjual.ForeColor = ColorTranslator.FromHtml("#FFFFFF");
            ...
            dtjual = new DataTable();
            query = "select jual_id, jual_tanggal, jual_nama_customer, jual_harga_total from jual order by 1 desc limit 1;";
            sqlCommand = new MySqlCommand(query, sqlConnect);
            sqlDataAdapter = new MySqlDataAdapter(sqlCommand);
            sqlDataAdapter.Fill(dtjual);
        }

        private void InvoiceSale_Load(object sender, EventArgs e)
        {
            ...
            lb_idjual.Text = "#" + dtjual.Rows[0][0].ToString();
            tb_customer.Text = dtjual.Rows[0][2].ToString();
            DateTime date = (DateTime)dtjual.Rows[0][1];
            lb_date.Text = date.ToString("dd/MM/yyyy");
            tb_total.Text = "TOTAL " + Convert.ToInt32(dtjual.Rows[0][3]).ToString("C", new CultureInfo("id-ID")); ;

            dtpakaian = new DataTable();
            query = "select * from vInvoiceSale;";
            sqlCommand = new MySqlCommand(query, sqlConnect);
            sqlDataAdapter = new MySqlDataAdapter(sqlCommand);
